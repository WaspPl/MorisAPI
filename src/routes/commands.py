...
async def get_commands(session: SessionDep, current_user: Annotated[User, Depends(get_current_user)], limit: int = 100, offset: int = 0, descending: bool = True, searchQuery: str = ''):
    commands = session.exec(select(Command).join(Command_Role_Assignment).where(Command_Role_Assignment.role_id == current_user.role_id).where(or_(Command.name.ilike(f"%{searchQuery}%"), Command.description.ilike(f"%{searchQuery}%"))).limit(limit).offset(offset).order_by(desc(Command.id) if descending else asc(Command.id))).all()
    return commands
